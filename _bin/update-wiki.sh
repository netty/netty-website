  ...
  if [[ ${LINE} =~ (/netty/netty/wiki(/([A-Z][-\._A-Za-z0-9]+))?)\"\>([^\<]+) ]]; then
    ...
    echo Retrieving: "$WIKI_FILE" from "$WIKI_URI" ..
    {
      ...
      echo ':plain'
      {
        ...
        | perl -p0 -e 's/[\r]//g' \
        | perl -p -e 's/^/  /g' \
        | perl -p -e 's/<a [^>]*absent[^>]*>(((?!<\/a>).)*)<\/a>/<span class="broken-link">$1<\/span>/gi' \
        | perl -p -e 's#/netty/netty/wiki/(Home)?"#index.html"#gi' \
        | perl -p -e 's#/netty/netty/wiki/((?!images/)[^"]+)#\L$1.html#g' \
        | perl -p -e 's/<table>/<table class="table table-hover">/gi' \
        || exit 1
      echo
